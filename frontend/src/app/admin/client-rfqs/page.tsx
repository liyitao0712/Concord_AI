...
const RFQ_STATUSES: Record<string, { label: string; color: string }> = {
  ...
  quoted: { label: '已报价', color: 'bg-green-100 text-green-800' },
  closed: { label: '已关闭', color: 'bg-emerald-100 text-emerald-800' },
...
function getStatusBadge(status: string) {
  const config = RFQ_STATUSES[status];
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold">客户询价</h1>
          <p className="mt-1 text-sm text-muted-foreground">管理客户询价单</p>
        </div>
        {can('client_rfq', 'create') && (
      ...
      <div className="flex gap-4">
        ...
        >
          ...
          {Object.entries(RFQ_STATUSES).map(([value, { label }]) => (
      ...
      <Card className="py-0">
        {loading ? (
          ...
          <div className="p-8 text-center text-muted-foreground">暂无客户询价单数据</div>
        ) : (
          <Table>
            <TableHeader>
              <TableRow>
      ...
      <Dialog open={showCreateForm} onOpenChange={(open) => !open && setShowCreateForm(false)}>
        <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-2xl">
          <DialogHeader>
            <DialogTitle>新建客户询价单</DialogTitle>
          </DialogHeader>
          <form onSubmit={handleCreate} className="space-y-6">
            ...
            <div className="flex justify-end space-x-3 pt-2">
              ...
              <Button type="submit" disabled={createLoading || !createForm.customer_id?.trim()}>
                {createLoading ? '创建中...' : '创建询价单'}
              </Button>
            </div>
          </form>
        </DialogContent>
            ...
            <>
              ...
              <div className="grid grid-cols-2 gap-4 text-sm">
                ...
                <div>
                  ...
                  <span>{formatDate(detailData.rfq_date)}</span>
                </div>
                {detailData.deadline && (
                  <div>
                    <span className="text-muted-foreground">报价截止：</span>
                    <span>{formatDate(detailData.deadline)}</span>
                  </div>
                )}
