...
const EmailsPanel = dynamic(
  () => import('@/app/admin/emails/EmailsPanel').then(mod => ({ default: mod.EmailsPanel })),
  { loading: () => <PageLoading /> }
);
const ProjectsPanel = dynamic(
  () => import('@/app/admin/projects/ProjectsPanel').then(mod => ({ default: mod.ProjectsPanel })),
  { loading: () => <PageLoading /> }
);
...
export default function DashboardPage() {
  return (
    <div className="space-y-6">
      {/* 页面标题 */}
      <div>
        <h1 className="text-2xl font-bold">牛批工作台</h1>
        <p className="mt-1 text-sm text-muted-foreground">高效工作，从这里开始</p>
      </div>
    ...
    return (
      <Card>
        <CardContent className="py-8 text-center">
          ...
          <button onClick={loadBriefing} className="mt-3 text-sm text-blue-600 hover:underline">
            重试
          </button>
        </CardContent>
      </Card>
            ...
            <div className="flex items-center gap-2">
              ...
              <div className="relative" ref={providerMenuRef}>
                <button
                  ...
                  className="flex items-center gap-1 text-sm px-2.5 py-1.5 rounded-md border border-input bg-background hover:bg-muted transition-colors"
                >
                ...
                {showProviderMenu && (
                  <div className="absolute right-0 top-full mt-1 z-50 bg-popover border rounded-lg shadow-lg py-1 min-w-[130px]">
                    {TTS_PROVIDER_OPTIONS.map((opt) => (
                      <button
                        ...
                        className={`w-full text-left px-3 py-1.5 text-sm hover:bg-muted transition-colors ${
                          ttsProvider === opt.value ? 'text-blue-600 font-medium' : ''
                        }`}
                      >
                        {opt.label}
                      </button>
                    ...
                    <div className="absolute right-0 top-full mt-1 z-50 bg-popover border rounded-lg shadow-lg py-1 min-w-[100px]">
                      {AUTO_REFRESH_OPTIONS.map((opt) => (
                        <button
                          key={opt.value}
                          onClick={() => {
                            setAutoRefreshSec(opt.value);
                            setShowRefreshMenu(false);
                          }}
                          className={`w-full text-left px-3 py-1.5 text-sm hover:bg-muted transition-colors ${
                            autoRefreshSec === opt.value ? 'text-blue-600 font-medium' : ''
                          }`}
                        >
                          {opt.label}
                        </button>
        ...
        <div className="flex gap-3">
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center gap-1.5 mt-2 flex-wrap">
              ...
              {/* 跳转邮件详情 */}
              <a
                href={`/admin/emails?highlight=${item.email_id}`}
