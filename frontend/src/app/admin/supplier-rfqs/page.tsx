...
const RFQ_STATUSES: Record<string, { label: string; color: string }> = {
  ...
  replied: { label: '已回复', color: 'bg-cyan-100 text-cyan-800' },
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold">供应商询价</h1>
          <p className="mt-1 text-sm text-muted-foreground">管理供应商询价单</p>
        </div>
      ...
      <Card className="py-0">
        {loading ? (
          ...
          <div className="p-8 text-center text-muted-foreground">暂无供应商询价单数据</div>
        ) : (
          <Table>
            <TableHeader>
              <TableRow>
      ...
      <Dialog open={showCreateForm} onOpenChange={(open) => !open && setShowCreateForm(false)}>
        <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-2xl">
          <DialogHeader>
            <DialogTitle>新建供应商询价单</DialogTitle>
          </DialogHeader>
