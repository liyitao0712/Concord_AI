...
export default function AgentsPage() {
  ...
  const resetPromptToDefault = async (promptName: string, type: 'system' | 'user') => {
    ...
    try {
      ...
      toast.success(`${type === 'system' ? 'System' : 'User'} Prompt 已重置为默认值`);
      await loadData();
    } catch (error: any) {
  ...
  return (
    <div className="space-y-6">
                        ...
                        <div className="sm:flex space-x-4 text-sm text-muted-foreground">
                          ...
                          {prompt && (
                            <p className="flex items-center text-xs text-muted-foreground">
                              v{prompt.version} · 更新于 {formatDate(prompt.updated_at)}
                            </p>
                          )}
                        </div>
                      </div>
      ...
      {/* 编辑模态框 */}
      <FormDialog
        open={!!editingAgent && !!currentAgent}
        onOpenChange={(open) => { if (!open) closeEditModal(); }}
        title={currentAgent?.display_name || currentAgent?.name || ''}
