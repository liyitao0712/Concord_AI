// src/app/admin/client-rfqs/page.tsx
// 客户询价单管理页面
//
// 功能说明：
// 1. 客户询价单列表（搜索、状态筛选、分页）
// 2. 新建客户询价单（简版表单）
// 3. 询价单详情查看 + 状态变更
...
import { clientRfqsApi, ClientRFQ, ClientRFQCreate } from '@/lib/api';
...
const RFQ_STATUS_MAP: Record<string, StatusConfig> = {
  draft: { label: '草稿', color: 'draft' },
  received: { label: '已接收', color: 'confirmed' },
  reviewing: { label: '审核中', color: 'pending' },
  quoted: { label: '已报价', color: 'completed' },
  closed: { label: '已关闭', color: 'active' },
  cancelled: { label: '已取消', color: 'cancelled' },
};

// 保留 label 查找（用于 toast 和状态变更按钮）
const RFQ_STATUSES: Record<string, { label: string }> = {
  draft: { label: '草稿' },
  received: { label: '已接收' },
  reviewing: { label: '审核中' },
  quoted: { label: '已报价' },
...
function safeDateFormat(dateStr: string | null): string {
  if (!dateStr) return '-';
  try {
...
export default function ClientRFQsPage() {
  ...
  const [rfqs, setRFQs] = useState<ClientRFQ[]>([]);
  ...
  const [createForm, setCreateForm] = useState<ClientRFQCreate>({
    customer_id: '',
    currency: 'USD',
    trade_term: '',
    payment_method: '',
  ...
  const loadRFQs = useCallback(async () => {
    ...
    setError(null);
    try {
      const response = await clientRfqsApi.list({
        page,
        page_size: PAGE_SIZE,
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await clientRfqsApi.create(cleaned as unknown as ClientRFQCreate);
  ...
  const handleDelete = async (rfq: ClientRFQ) => {
    ...
    if (!confirmed) return;
    try {
      await clientRfqsApi.delete(rfq.id);
  ...
  const loadDetail = async (id: string) => {
    ...
    try {
      const detail = await clientRfqsApi.get(id);
  ...
  const openDetail = (rfq: ClientRFQ) => {
    setShowDetail(true);
    loadDetail(rfq.id);
  };
  ...
  const handleStatusChange = async (newStatus: string) => {
    ...
    try {
      await clientRfqsApi.updateStatus(detailData.id, newStatus);
      toast.success(`状态已更新为「${RFQ_STATUSES[newStatus]?.label || newStatus}」`);
        ...
        filters={[
          {
            key: 'status',
            placeholder: '全部状态',
            options: Object.entries(RFQ_STATUSES).map(([value, { label }]) => ({ value, label })),
          },
        ]}
        ...
        <TableBody>
          {rfqs.map((rfq) => (
            <TableRow key={rfq.id}>
              ...
              <TableCell className="px-6 py-4 text-sm text-muted-foreground">
                {safeDateFormat(rfq.rfq_date)}
              </TableCell>
              <TableCell className="px-6 py-4">
                <StatusBadge status={rfq.status} statusMap={RFQ_STATUS_MAP} fallbackLabel={rfq.status} />
              </TableCell>
            ...
            <>
              <DialogHeader>
                ...
                <div className="flex items-center gap-3 mt-2">
                  <StatusBadge status={detailData.status} statusMap={RFQ_STATUS_MAP} fallbackLabel={detailData.status} />
              ...
              <FormSection title="状态变更" collapsible={false}>
                <div className="flex flex-wrap gap-2">
                  {Object.entries(RFQ_STATUSES).map(([statusKey, { label }]) => (
              ...
              <div className="grid grid-cols-2 gap-4 text-sm">
                ...
                <div>
                  ...
                  <span>{safeDateFormat(detailData.rfq_date)}</span>
                </div>
                {detailData.deadline && (
                  <div>
                    <span className="text-muted-foreground">报价截止：</span>
                    <span>{safeDateFormat(detailData.deadline)}</span>
                  </div>
                )}
                {detailData.expiry_date && (
                  <div>
                    <span className="text-muted-foreground">有效期：</span>
                    <span>{safeDateFormat(detailData.expiry_date)}</span>
                  </div>
                )}
                ...
                <div>
                  ...
                  <span>{safeDateFormat(detailData.created_at)}</span>
                </div>
                <div>
                  <span className="text-muted-foreground">更新时间：</span>
                  <span>{safeDateFormat(detailData.updated_at)}</span>
                </div>
