...
class SupplierQuotationLineCreate(BaseModel):
    """创建供应商报价明细行"""
...
class SupplierQuotationLineResponse(BaseModel):
    """供应商报价明细行响应"""
    id: str
    supplier_quotation_id: str
    line_no: int
    product_id: str
    product_name: str
...
SUPPLIER_QUOTATION_STATUSES = {"draft", "received", "reviewing", "accepted", "rejected", "expired", "cancelled"}
...
class SupplierQuotationCreate(BaseModel):
    ...
    linked_supplier_rfq_id: Optional[str] = Field(None, description="来源供应商询价单 ID")
    ...
    lines: List[SupplierQuotationLineCreate] = Field(default=[], description="明细行列表")

    @field_validator("status")
    @classmethod
    def validate_status(cls, v):
        if v not in SUPPLIER_QUOTATION_STATUSES:
            raise ValueError(f"状态必须是: {', '.join(sorted(SUPPLIER_QUOTATION_STATUSES))}")
        return v
...
class SupplierQuotationResponse(BaseModel):
    ...
    linked_supplier_rfq_id: Optional[str] = None
    ...
    linked_supplier_rfq_no: Optional[str] = Field(None, description="来源询价单编号")
...
class SupplierQuotationDetailResponse(SupplierQuotationResponse):
    """供应商报价单详情响应（含明细行）"""
    lines: List[SupplierQuotationLineResponse] = Field(default=[], description="明细行列表")


class SupplierQuotationStatusUpdate(BaseModel):
    """变更供应商报价单状态"""
