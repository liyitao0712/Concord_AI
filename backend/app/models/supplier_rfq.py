# app/models/supplier_rfq.py
# 供应商询价单管理模型
#
# 功能说明：
# 1. SupplierRFQ - 供应商询价单表，记录我们向供应商的询价
# 2. SupplierRFQLine - 供应商询价单明细表，记录询价中的每个产品行项
#
# 使用方法：
...
class SupplierRFQ(Base):
    ...
    记录我们向供应商发出的询价请求，包含贸易条款、财务信息等。
    可流转为供应商报价单（SupplierQuotation）。
    ...
    status: Mapped[str] = mapped_column(
        String(20),
        default="draft",
        comment="状态: draft/sent/replied/quoted/closed/cancelled",
    )
    ...
    lines: Mapped[List["SupplierRFQLine"]] = relationship(
        ...
        order_by="SupplierRFQLine.line_no",
    )

    __table_args__ = (
        Index("ix_supplier_rfqs_rfq_no", "rfq_no"),
        Index("ix_supplier_rfqs_supplier_id", "supplier_id"),
        Index("ix_supplier_rfqs_status", "status"),
        Index("ix_supplier_rfqs_rfq_date", "rfq_date"),
    )

    def __repr__(self) -> str:
        return f"<SupplierRFQ {self.rfq_no}>"


class SupplierRFQLine(Base):
    """
    供应商询价单明细表

    记录供应商询价单中的每个产品行项，包含数量、单价、财务字段等。
    """

    __tablename__ = "supplier_rfq_lines"
    ...
    # ==================== 关联 ====================
    supplier_rfq_id: Mapped[str] = mapped_column(
        String(36),
        ForeignKey("supplier_rfqs.id", ondelete="CASCADE"),
        nullable=False,
        comment="供应商询价单 ID",
    )
    ...
    # ==================== 关系 ====================
    supplier_rfq: Mapped["SupplierRFQ"] = relationship(
        "SupplierRFQ",
    ...
    __table_args__ = (
        Index("ix_supplier_rfq_lines_supplier_rfq_id", "supplier_rfq_id"),
        Index("ix_supplier_rfq_lines_product_id", "product_id"),
    )

    def __repr__(self) -> str:
        return f"<SupplierRFQLine {self.line_no}: {self.product_name}>"
