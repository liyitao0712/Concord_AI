...
| Worker | 文件 | 功能 | 运行方式 |
|--------|------|------|----------|
| Celery Beat | `celery_app.py` | 定时任务调度（邮件拉取） | `celery -A app.celery_app beat` |
| Celery Worker | `tasks/email.py` | 异步任务执行（邮件处理） | `celery -A app.celery_app worker` |
...
│                                                                 │
│  ┌─────────────────┐                                           │
│  │  Celery Beat    │ ──► 定时调度器 (60s 间隔)                  │
│  │                 │     └─► 动态创建邮箱拉取任务                │
│  │                 │         └─► 发送到 Redis 队列               │
│  └─────────────────┘                                           │
│                                                                 │
│  ┌─────────────────┐                                           │
│  │ Celery Worker   │ ──► 并发任务执行器                         │
│  │  (可多实例)      │     └─► poll_email_account                │
│  │                 │         └─► process_email                 │
│  │                 │             └─► Persist & Dispatch        │
│  └─────────────────┘                                           │
│                                                                 │
...
```
┌──────────┐     ┌──────────┐     ┌──────────┐     ┌──────────┐
│  IMAP    │────►│  Celery  │────►│  Event   │────►│ Temporal │
│  Server  │     │  Tasks   │     │Dispatcher│     │ Workflow │
└──────────┘     └────┬─────┘     └────┬─────┘     └────┬─────┘
                      │                │                │
                      ▼                ▼                ▼
               ┌──────────┐     ┌──────────┐     ┌──────────┐
...
注：Celery Beat 定时触发 poll_email_account 任务，
    Worker 并发执行邮件拉取和处理
```
...
│  ┌─────────────────────────────────────────────────┐           │
│  │              Worker 进程 (独立运行)              │           │
│  │  • Temporal Worker (工作流引擎)                  │           │
│  └─────────────────────────────────────────────────┘           │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘

外部服务：
• 阿里云 OSS（文件存储，可选，降级到本地存储）
